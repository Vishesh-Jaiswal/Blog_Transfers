...
function GetReportedBlogs() {
  ...
  useEffect(() => {
    const fetchBlogs = async () => {
      try {
        const blogResponse = await axios.get('http://localhost:5273/api/Blog/ReportedBlogs');
        setReportedBlogs(blogResponse.data|| []);
    ...
    <div className="show-blogs">
      ...
      <div className="blog-list-container">
        <div className="blogContents">
          <h2 className="PageTitle">Reported Blogs</h2>
          {Array.isArray(reportedBlogs) && reportedBlogs.length > 0 ? (
            <ul>
              {reportedBlogs.map((blog) => (
                <li key={blog.blogId} className="blog-item">
                  <Link to={`/showreportedblog/${blog.blogId}`} className="blog-link">
...
export default GetReportedBlogs;